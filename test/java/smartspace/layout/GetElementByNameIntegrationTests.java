...
public class GetElementByNameIntegrationTests {
	...
	public void init() {
		this.baseUrl = "http://localhost:" + port
				+ "/smartspace/elements/{userSmartspace}/{userEmail}/?search=name&value={name}&page={page}&size={size}";
	...
	@Test
	public void testGetElementByNameWithManagerAndEmptyDB() throws Exception {
	...
	@Test
	public void testGetElementByNameWithManagerWithPagination() throws Exception {
	...
	public void testGetElementByNameWithWrongManagerWithPagination() throws Exception {
		// GIVEN the database contain manager and element
		UserEntity manager = faker.entity().user();
		manager.setRole(UserRole.MANAGER);
		this.userDao.create(manager);
	...
	@Test
	public void testGetElementByNameWithPlayerWithPagination() throws Exception {
	...
	@Test(expected = Exception.class)
	public void testGetElementByNameWithWrongPlayerWithPagination() throws Exception {
		// GIVEN the database contain player and element
		UserEntity player = faker.entity().user();
		player.setRole(UserRole.PLAYER);
		this.userDao.create(player);
		...
		// WHEN I GET element by name with wrong manager
	...
	@Test
	public void testGetElementNoExpiredByNameWithPlayerAndPagination() throws Exception {
		...
		elements=elements.stream().peek(element->element.setExpired(false)).peek(element->element.setName("test")).map(elementDao::create).collect(Collectors.toList());
		
		List<ElementEntity> elementsExpired = faker.entity().elementList(size);
		elementsExpired=elementsExpired.stream().peek(element->element.setExpired(true)).peek(element->element.setName("test")).map(elementDao::create).collect(Collectors.toList());
	...
	@Test
	public void testGetElementExpiredByNameWithManagerAndPagination() throws Exception {
	...
	public void testGetElementExpiredByNameWithManager() throws Exception {
		// GIVEN the database contain manager and 4 elements 
		UserEntity manager = faker.entity().user();
		manager.setRole(UserRole.MANAGER);
		this.userDao.create(manager);
		...
		List<ElementEntity> elementsExpired = faker.entity().elementList(size);
		elementsExpired=elementsExpired.stream().peek(element->element.setExpired(false)).peek(element->element.setName("test1")).map(elementDao::create).collect(Collectors.toList());
	...
	@Test(expected = Exception.class)
	public void testGetElementByNameWithAdmin() throws Exception {
		...
		// WHEN I GET element by name with admin
